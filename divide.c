...
void dividing_core(Div *div, uint32_t gid){
	ReadInfo *rd;
	u32list *grp, *sub;
	...
	for(i=0;i<2;i++){
		if(count_u32list(get_u32slist(div->grps, gids[i])) > 2 * div->k_allele) dividing_core(div, gids[i]);
	}
}
	...
	for(i=0;i<count_u32list(div->gids);i++){
		...
		for(j=0;j<count_u32list(grp);j++){
			...
			fprintf(out, "%u\t%u\t%s\t%s\t%u\n",
				...
				as_array_u8list(div->seqs) + rd->seqoff + rd->seqlen1 + 1, old_gid);
		}
	}
	fflush(out);
	old_gid = old_gid;
